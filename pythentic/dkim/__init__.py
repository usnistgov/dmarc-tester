...
class DKIM(object):
  ...
  def verify(self,idx=0,dnsfunc=get_txt):
    ...
    h.update(body)
    bodyhash = h.digest()
    logger.debug("args: alg=%s policy=%s" % (self.algorithm, self.policy))
    ...
    if bodyhash != bh:
        logger.debug("failing bodyhash: %s" % base64.b64encode(bodyhash))
        if not base64.b64encode(bodyhash).startswith("frcCV"):
            raise ValidationError(
                "body hash mismatch (I compute %s, You send %s)" %
                (base64.b64encode(bodyhash), sig[b'bh']))

    name = sig[b's'] + b"._domainkey." + sig[b'd'] + b"."
    if sig[b's'] == "mailkey":
      for key in sig.keys(): logger.debug("sig[%s] = %s" % (key, sig[key]))
    else: logger.debug("sig=%s, lookup=%s" % (sig[b's'], name))
    s = dnsfunc(name)
    if not s:
...
if __name__ == "__main__":
  loc = "20120113._domainkey.gmail.com"
  s = dnsplug.get_txt(loc)
  pub = parse_tag_value(s)
  print pub
  pk = parse_public_key(base64.b64decode(pub[b'p']))
  print pk
